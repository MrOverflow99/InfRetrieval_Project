...
//import ir.util.TextFilePreviewer;
...
import java.util.Scanner;
	...
	public class Main {
		private static final String DOCUMENTS_PATH = "src/main/resources/documents/archive/aclImdb/test/neg";
		 ...
		 * "src/main/resources/documents/archive/aclImdb/test/neg";
		 * TextFilePreviewer.printFirstFiveTextFiles(directoryPath);
		 */
	...
	private static void executeQuery(Scanner scanner, QueryProcessor standardProcessor,
		...
		if (result.isEmpty()) {
			...
			for (Posting posting : result) {
				...
				if (doc != null) {
					count++;
					System.out.println(count + ". " + doc.getName() + " (ID: " + doc.getId() + ", Score: "
	...
	private static void showIndexStatistics(Dictionary dictionary) {
		...
		String[] mostFrequentTerms = dictionary.getTerms().stream().sorted((t1, t2) -> {
